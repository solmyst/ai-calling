...
# Model-failure rules every domain shares — see guardrails.py.
...
class PriceGuard:
    ...
    # (list attribute, log level, what bot.py's GuardFilter logs when it grows).
    # WARNING only observes; ERROR means the caller never heard the original.
    # Moved here from bot.py 2026-09-23 — the rules are this domain's, not the
    # pipeline's.
    COUNTERS = (
        ("blocked", "error", "blocked invented price(s)"),
        ("odd_times", "warning", "saw a non-standard slot time"),
        ("false_confirmations", "error", "rewrote a booking claimed as done"),
        ("romanised", "warning", "SCRIPT DRIFT, romanised (not rewritten)"),
        ("labels", "error", "stripped a leaked speaker label"),
        ("availability_claims", "error", "rewrote a slot/area availability claim"),
        ("damage_promises", "error", "rewrote a damage/liability promise"),
        ("body_type_asks", "error", "rewrote a hatchback/sedan/SUV question"),
        ("self_ai", "error", "rewrote the bot announcing it is an AI"),
        ("machine_output", "error", "DROPPED non-speech output"),
        ("priced_before_car", "error", "blocked a price before the caller named a car"),
...
def _demo():
    ...
    if domain.ACTIVE != "car_spa":
        raise SystemExit(
            f"this is the car spa's guard, but DOMAIN={domain.ACTIVE!r}.\n"
            f"  run:  DOMAIN=car_spa python -m domains.car_spa.guard\n"
    ...
    # Every counter bot.py logs must be a list this guard actually keeps.
    for name, level, _ in PriceGuard.COUNTERS:
        assert isinstance(getattr(g, name), list) and level in ("error", "warning"), name

    print("car spa guard ok — allowed prices:", sorted(g.allowed))
