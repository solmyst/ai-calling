...
- **Structured conversations (Pipecat Flows)** — when the conversation has distinct *stages* with different rules (intake → order → payment → confirmation; branching by intent), don't pile every instruction and tool into one giant system prompt. **Pipecat Flows** (`pipecat.flows`, part of Pipecat) layers a state machine over the pipeline: each node gets its own prompt, tools, and actions, with explicit transitions. Routing heuristic: a single context + tools (above) is right for most bots; reach for Flows when the prompt/tool set must *change per stage* or the process must be enforced, not suggested. Confirm the APIs via §3 before writing Flows code.
  - **Default to YAML for a new flow.** Write the graph as a `FlowConfig` YAML file — the nodes, each node's messages, which tools it offers, and where each tool leads — and the tools as a Python module of direct functions (`async def choose_pizza(flow_manager, size: str): ...` returning `(result, TRANSITION_IN_YAML)`; the docstring is the tool's description). The config decides the next node, by name or with a branch table keyed on a field of the tool's result, so tools never pick nodes. A function that only moves the conversation needs no Python: write it as `- name: revise_order` with `transition_only: true`, a `description` for the LLM, and `transition_to`. Python tools are for functions that capture data or do work. When a transition depends on logic, keep the business logic flow-agnostic and write a thin tool that calls it and reports the outcome as a field (`status: available`) for the config to branch on. Construct `Flow(config, handlers=module)`, where the module holds the direct functions and any action handlers, and hand `flow.initial_node` and `flow.global_functions` to `FlowManager`. Constructing the `Flow` checks every tool, handler, and variable the config names and reports all the misses together, so start the bot once as a smoke test before running evals. Find the `food_ordering` and `restaurant_reservation` YAML examples via §3 and copy their shape.
  - **Use Python for the flow when it needs code:** the next node depends on logic a branch table can't express, a node's prompt is assembled from runtime data, or nodes are generated from a dataset. Tools are the same direct functions either way, and moving a flow between YAML and Python is mechanical.
...
<!-- Generated by `pipecat init` (pipecat-ai 1.10.0). Run `pipecat init --overwrite-guide` to refresh. -->
